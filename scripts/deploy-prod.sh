...
deploy_code() {
    ...
    run_on_vmss "$APP_VMSS" "git config --global --add safe.directory /opt/cashflip/app 2>/dev/null; su - cashflip -c 'git config --global --add safe.directory /opt/cashflip/app' 2>/dev/null; bash /opt/cashflip/deploy.sh" "Deploy code"

    log "Deploying code to Celery VMs..."
    run_on_vmss "$CELERY_VMSS" "git config --global --add safe.directory /opt/cashflip/app 2>/dev/null; su - cashflip -c 'git config --global --add safe.directory /opt/cashflip/app' 2>/dev/null; bash /opt/cashflip/deploy.sh" "Deploy code"
