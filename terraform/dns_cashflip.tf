# Porkbun DNS Automation for cashflip.cash
# 
# After `terraform apply`, run this script to update DNS:
#
#   ./update_dns_cashflip.sh $(terraform output -raw lb_public_ip)
#
# Requires PORKBUN_API_KEY and PORKBUN_SECRET_KEY environment variables
# (set in production.env)

# DNS update script for cashflip.cash domain
resource "local_file" "dns_cashflip_script" {
  filename = "${path.module}/update_dns_cashflip.sh"
  content  = <<-SCRIPT
    #!/bin/bash
